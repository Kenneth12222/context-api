        ...
        <div className='shopping-store'>
            ...
            {isCartOpen && (
                <div className='cart-container'>
                    <div className='cart'>
                        <a className='close-button' onClick={toggleCart}>
                            &times;
                        </a>
                        <h1 className="text-2xl font-semibold text-gray-900">Your Cart</h1>
                        {cart.map((item) => (
                            <div className="mx-auto px-4 sm:px-6 lg:px-8" key={item.id}>
                                                                ...
                                                                <div className="mt-4 flex items-end justify-between sm:mt-0 sm:items-start sm:justify-end">
                                                                    <div className="sm:order-1">
                                                                        <div className="mx-auto flex h-8 items-stretch text-gray-600">
                                                                            <button className="flex items-center justify-center rounded-l-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" onClick={() => decreaseQuantity(item.id)}>-</button>
                                                                            <div className="flex w-full items-center justify-center bg-gray-100 px-4 text-xs uppercase transition">{item.quantity}</div>
                                                                            <button className="flex items-center justify-center rounded-r-md bg-gray-200 px-4 transition hover:bg-black hover:text-white" onClick={() => increaseQuantity(item.id)}>+</button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                        ...
                        <div className="max-w-[95%]">
                            ...
                            <div className="mt-6 text-center max-w-[60%]">
                                <button type="button" className="group inline-flex w-full items-center justify-center rounded-md bg-gray-900 px-6 py-4 text-lg font-semibold text-white transition-all duration-200 ease-in-out focus:shadow hover:bg-gray-800 max-w-[55%]">
                                    Checkout
